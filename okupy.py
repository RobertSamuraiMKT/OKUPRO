...
st.set_page_config(page_title="OKUPRO v7.0", layout="wide", page_icon="🏠")
...
def generar_pdf_inmueble(row):
    ...
    story.append(Paragraph("🏠 OKUPRO v7.0 — Ficha de inmueble", title_style))
    ...
    story.append(Paragraph("OKUPRO v7.0 · Uso exclusivamente informativo · No constituye asesoramiento financiero", footer_style))
    doc.build(story)
    buf.seek(0)
    return buf
...
# Columnas candidatas para la deuda (OB). NO incluimos tasación/precio de tasación.
CANDIDATAS_DEUDA = ["ob", "deuda", "saldo", "outstanding", "balance", "principal", "importe deuda", "deuda pendiente"]
EXCLUIR_DEUDA    = ["tasacion", "tasación", "subasta", "precio", "pvp", "valor", "importe venta"]

def detectar_columna_deuda(df):
    """
    Busca la columna de deuda con lógica específica:
    1. Primero busca 'OB' exacto (case-insensitive)
    2. Luego busca variantes de 'deuda'
    3. Nunca confunde con tasación/precio
    Devuelve (col, es_ambiguo) donde es_ambiguo=True si hay que preguntar al usuario.
    """
    ...
    # Paso 2: candidatas directas (sin palabras excluidas)
    candidatas = []
    for col, lower in cols_lower.items():
        if any(cand in lower for cand in CANDIDATAS_DEUDA):
            if not any(excl in lower for excl in EXCLUIR_DEUDA):
                candidatas.append(col)

    if len(candidatas) == 1:
        return candidatas[0], False
    elif len(candidatas) > 1:
        return candidatas, True  # ambiguo: hay que preguntar

    return None, False  # no encontrada


def clasificar_ltv(ltv):
    """Semáforo de inversión según LTV (OB / Valor Mercado)"""
    if ltv <= 0.5:
...
def procesar_npl(df, col_deuda, precios_usuario, vuln_dict, descuento_compra_pct):
    """
    Procesa el Excel NPL y calcula:
    - Valor de mercado estimado
    - LTV (OB / Valor mercado)
    - Margen bruto inversor (Valor mercado - OB)
    - Precio compra recomendado (OB * (1 - descuento_compra))
    - Rentabilidad estimada sobre precio de compra
    """
    col_id     = encontrar_columna(df, ["id", "prinex", "expediente", "id prinex"])
    col_ref    = encontrar_columna(df, ["catastral", "referencia catastral", "cd referencia"])
    col_dir    = encontrar_columna(df, ["dirección", "direccion", "domicilio", "calle", "dirección completa"])
    col_cp     = encontrar_columna(df, ["postal", "cp", "código postal", "codigo postal"])
    col_mun    = encontrar_columna(df, ["municipio", "poblacion", "población", "ciudad"])
    col_ccaa   = encontrar_columna(df, ["ccaa", "comunidad", "autonomia", "autonomía"])
    col_tipo   = encontrar_columna(df, ["tipo inmueble", "tipo", "clase"])
    col_pl_npl = encontrar_columna(df, ["pl npl", "pl_npl", "tipo cartera"])
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Tipo de inmueble desde columna o inferencia
            ...
            # Para viviendas sin superficie usamos precio medio de zona (estimación por OB/LTV inverso)
            # Usamos un m² estándar de 80m² para VIVIENDA, 150m² para CASA
            m2_estimado = 80 if tipo_label == "VIVIENDA" else 150
            precio_mercado_est = m2_estimado * mercado["venta"]

            # LTV y métricas clave
...
def generar_pdf_npl(row):
    ...
    alert_style  = ParagraphStyle("alert", fontSize=12, fontName="Helvetica-Bold", spaceAfter=10)

    story.append(Paragraph("📋 OKUPRO v7.0 — Ficha NPL / Crédito Hipotecario", title_style))
    ...
    story.append(Spacer(1, 0.2*cm))
    story.append(Paragraph(
        "OKUPRO v7.0 · Módulo NPL · Uso exclusivamente informativo · No constituye asesoramiento financiero ni legal",
        footer_style))
    doc.build(story)
    buf.seek(0)
    return buf
...
with tab_cartera:
    ...
    if archivo is not None:
        try:
            ...
            if umbral_roi  > 0: df_filtrado = df_filtrado[df_filtrado["ROI Flip (%)"] >= umbral_roi]
            if umbral_rent > 0: df_filtrado = df_filtrado[df_filtrado["Rent. bruta (%)"] >= umbral_rent]
            if solo_riesgo_bajo: df_filtrado = df_filtrado[df_filtrado["Riesgo zona"] == "BAJA"]
            if solo_revisar:     df_filtrado = df_filtrado[df_filtrado["Riesgo zona"] == "REVISAR"]
...
with tab_npl:
    ...
    if archivo_npl is not None:
        try:
            ...
            # ── Detección columna deuda ──────────────────────────────
            col_deuda_result, es_ambiguo = detectar_columna_deuda(df_npl_raw)

            if es_ambiguo:
                # Hay varias candidatas → preguntamos al usuario
            ...
            # Previsualización de la columna deuda
            if col_deuda:
            ...
            if st.button("🚀 Analizar cartera NPL", type="primary"):
                with st.spinner("⏳ Procesando créditos NPL..."):
                    df_npl_res = procesar_npl(df_npl_raw, col_deuda,
                                              precios_usuario_npl if 'precios_usuario_npl' in dir() else None,
...
st.markdown("---")
st.caption("OKUPRO v7.0 · Cartera OkuPro + Módulo NPL · Scoring multidimensional · LTV & Margen inversor · PDF por inmueble")
